// All SFX synthesized with Web Audio. No assets.
let ctx = null;
let master = null;
let whiteBuf = null;
let pinkBuf = null;
...
export function sfxBounce(speed, surfaceId) {
  if (!ctx) return;
...
export function sfxCrowd(intensity) {
  if (!ctx) return;
  const t = ctx.currentTime;
  const dur = 1.8;
